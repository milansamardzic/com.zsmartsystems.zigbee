...
public class ZigBeeDongleTelegesis
    ...
    public void sendCommand(final ZigBeeApsFrame apsFrame) {
        ...
        new Thread() {
            @Override
            public void run() {
                logger.debug("------- sending  frame {}", command);

                frameHandler.sendRequest(command);

                logger.debug("------- complete frame {}", command);
                ...
                int messageId = ((TelegesisSendUnicastCommand) command).getMessageId();
                messageIdMap.put(messageId, apsFrame.getApsCounter());

                logger.debug("------- complete frame --- adding map {}  {}", messageId, apsFrame.getApsCounter());
            }
        }.start();
    }
    ...
    public void telegesisEventReceived(TelegesisEvent event) {
        ...
        if (event instanceof TelegesisAckMessageEvent) {
            ...
            logger.debug("*** TelegesisAckMessageEvent {}", ackEvent.getMessageId());
            ...
            logger.debug("*** TelegesisAckMessageEvent {}  {}", ackEvent.getMessageId(),
                    messageIdMap.get(ackEvent.getMessageId()));
